 ...
 * @version: 
 * @Author: LZW
 * @Date: 2023-08-04 21:41:02
 * @LastEditors: LZW
 * @LastEditTime: 2023-08-13 19:48:53
 */
/**
	************************************************************
	************************************************************
		...
		if(dataPtr != NULL)
		{
			if(MQTT_UnPacketRecv(dataPtr) == MQTT_PKT_CONNACK)
			{
				switch(MQTT_UnPacketConnectAck(dataPtr))
				{
					...
					case 2:Usart_Printf(USART_DEBUG, "WARN:	连接失败：非法的clientid\r\n");break;
					case 3:Usart_Printf(USART_DEBUG, "WARN:	连接失败：服务器失败\r\n");break;
					case 4:Usart_Printf(USART_DEBUG, "WARN:	连接失败：用户名或密码错误\r\n");break;
					case 5:Usart_Printf(USART_DEBUG, "WARN:	连接失败：非法链接(比如token非法)\r\n");break;
...
//	memset(text, 0, sizeof(text));
//	sprintf(text, "Temperature,%.1f;", SHT30_Temperature);
//	strcat(buf, text);
...
void OneNet_SendData(void)
{
	...
	Usart_Printf(USART_DEBUG, "Tips:	OneNet_SendData-MQTT\r\n");
	...
	if(body_len)
	{
		if(MQTT_PacketSaveData(DEVID, body_len, NULL, 5, &mqttPacket) == 0)							//封包
		{
			...
			Usart_Printf(USART_DEBUG, "Send %d Bytes\r\n", mqttPacket._len);
			
			MQTT_DeleteBuffer(&mqttPacket);															//删包
		}
		else
			Usart_Printf(USART_DEBUG, "WARN:	EDP_NewBuffer Failed\r\n");
	}
...
void OneNet_RevPro(unsigned char *cmd)
{
	...
	switch(type)
	{
		case MQTT_PKT_CMD:															//命令下发
			...
			if(result == 0)
			{
				...
				{
					Usart_Printf(USART_DEBUG, "Tips:	Send CmdResp\r\n");
		...
		case MQTT_PKT_PUBLISH:														//接收的Publish消息
			...
			if(result == 0)
			{
				Usart_Printf(USART_DEBUG, "topic: %s, topic_len: %d, payload: %s, payload_len: %d\r\n",
				...
				switch(qos)
				{
					case 1:															//收到publish的qos为1，设备需要回复Ack
						...
						{
							Usart_Printf(USART_DEBUG, "Tips:	Send PublishAck\r\n");
					...
					case 2:															//收到publish的qos为2，设备先回复Rec
						...
						{
							Usart_Printf(USART_DEBUG, "Tips:	Send PublishRec\r\n");
							ESP8266_SendData(mqttPacket._data, mqttPacket._len);
							MQTT_DeleteBuffer(&mqttPacket);
						}
	...
	if(dataPtr != NULL && result != -1)					//如果找到了
	{
        ...
        OneNet_HandleData( req_payload, num );
	}
...
{
    printf( "num:%d\r\n",num);
    if(strstr((char *)req_payload, "LED1"))		//搜索"redled"
    {
        //LED
        if( num == 0 )
            LED1 = 0;
